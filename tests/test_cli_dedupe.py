...
def test_run_crawl_skips_new_unavailable_products(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli, "load_store_seeds_from_dir", lambda path, _settings: [seed])
    ...
    # Image re-hosting was removed: the scraper never uploads to Supabase.
    assert _FakeUploader.uploaded_for == []
...
def test_run_crawl_constructs_direct_db_repository(monkeypatch) -> None:
    settings = Settings(
        LOG_LEVEL="INFO",
        SUPABASE_URL="https://x.supabase.co",
        SUPABASE_SERVICE_ROLE_KEY="key",
...
def test_run_crawl_db_first_resume_processes_pending_only(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli, "get_settings", lambda: settings)
    monkeypatch.setattr(cli, "_run_orphan_preflight", lambda _settings, batch_size=200: None)
    monkeypatch.setattr(cli, "load_store_seeds_from_dir", lambda path, _settings: csv_seeds)
